...
- Mine tab offers Merge via `PullRequestStore.canOfferMergeForMine` (`canMerge` AND review approved/nil — approvals come from others since authors can't approve own PRs, and branch protection requiring reviews must be satisfied first); Review tab still uses `canOfferMergeInPlace` (approved + `canMerge`) and `shouldOfferApprove`
...
- Auto-approved history rows carry live merge readiness: `PullRequestStore.historyStatuses` (`HistoryPRStatus`: merged, reviewDecision, mergeable, viewerPermission, head ref) refreshes via the `HistoryStates` `nodes(ids:)` query on every live `refresh(force:)`; a row shows `Merged` once GitHub reports it merged, otherwise offers Merge when not merged AND review is approved/nil AND `canMerge` (WRITE+ and MERGEABLE) — this is the merge home for approved PRs that GitHub drops from the `review-requested:@me` search after review; `mergedHistoryIDs` is derived from the same statuses; failures keep the previous set and an empty history skips the call
